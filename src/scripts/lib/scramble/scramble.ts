import { shuffle, identity, split, pipe, slice, reverse } from "./fp"
...
export const scrambleCharWithCharSet = (charset: string[], preserveCasing?: boolean) => {
  function scrambleChar(char: string): string {
    if (char.length > 1) throw new Error(`You can only scramble one character at a time. Received: ${char}`)
...
export const scramble = (text: string, userOptions?: Partial<ScrambleOptions>): string => {
  const config = {
    ...defaultOptions,
    ...userOptions,
  }
  ...
  const characterIsUnscrambled = (index: number): boolean => config.previousText.charAt(index) === text.charAt(index)
